...
export function LincolnHeightsEngine({ playKey, onHud, onFinish }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchWind = async () => {
      try {
        const r = await fetch(
          "https://api.open-meteo.com/v1/forecast?latitude=34.0724&longitude=-118.2177&current=wind_speed_10m,wind_direction_10m,wind_gusts_10m"
        );
        const j = await r.json();
        if (cancelled) return;
        const c = j.current ?? {};
        const w: Wind = {
          speed: c.wind_speed_10m ?? 2,
          direction: c.wind_direction_10m ?? 270,
          gusts: c.wind_gusts_10m ?? 3,
        };
        stateRef.current.wind = w;
        onHud({
          windSpeed: `${(w.speed * 3.6).toFixed(1)} km/h`,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(
          "https://newsapi.org/v2/everything?q=%22Lincoln+Heights%22+%22Los+Angeles%22&language=en&sortBy=publishedAt&pageSize=8&apiKey=7a465e6a1c994a47af01a22a82d8f362"
        );
        const j = await r.json();
        if (cancelled) return;
        const arts = (j.articles ?? []) as Array<{ source: { name: string }; title: string }>;
        const n = LINCOLN_HEIGHTS_GPS.length;
        const items: NewsItem[] = arts.slice(0, 8).map((a, i) => ({
          source: a.source?.name ?? "News",
          title: a.title ?? "",
          idx: Math.floor(((i + 1) * n) / 9),
        }));
        stateRef.current.news = items;
      } catch { /* ignore */ }
    })();
  ...
  async function start() {
    ...
    // Schedule segments
    const pts = LINCOLN_HEIGHTS_GPS;
    const total = stateRef.current.maxDist;
    let elapsed = 0;
    for (let i = 1; i < pts.length; i++) {
      const seg = pts[i];
      const dur = Math.max(30, (seg.dist / total) * TOTAL_MS);
      const at = elapsed;
      const t = setTimeout(() => stepSegment(i, dur / 1000), at);
      stateRef.current.timeouts.push(t);
      elapsed += dur;
    }
    const tEnd = setTimeout(() => finish(), elapsed + 200);
    stateRef.current.timeouts.push(tEnd);
  ...
  function stepSegment(i: number, segSec: number) {
    ...
    const cum = stateRef.current.cumDist;
    const total = stateRef.current.maxDist;
    const prev = proj[i - 1], cur = proj[i];
    const seg = pts[i];
    const cumNow = cum[i];
    const normDist = cumNow / total;
    ...
    oc.lineCap = "round";
    oc.beginPath();
    oc.moveTo(prev.x, prev.y);
    oc.lineTo(cur.x, cur.y);
    oc.stroke();
    oc.restore();
    ...
    if (a) {
      ...
      const lfoRate = mapRange(stateRef.current.wind.speed, 0, 15, 0.2, 3);
    ...
    // HUD
    onHud({
      frequency: `${Math.round(seg.ele)} Hz`,
      cutoff: `${Math.round(mapRange(cumNow, 0, total, 200, 3000))} Hz`,
      elevation: `${seg.ele} m`,
    ...
    if (news) {
      stateRef.current.activeCards.push({ idx: i, news, age: 0, maxAge: 360 });
    }
  }

  function render() {
    const main = mainRef.current; const off = offRef.current;
    if (!main || !off) return;
    const ctx = main.getContext("2d")!;
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      const trailLen = Math.max(12, stateRef.current.wind.speed * 3.5);
      if (p.trail.length > trailLen) p.trail.shift();
      const lifeT = p.life / p.maxLife;
      const alpha = lifeT < 0.2 ? lifeT / 0.2 : lifeT > 0.8 ? (1 - lifeT) / 0.2 : 1;
      const a = Math.max(0, Math.min(1, alpha)) * 0.55;
    ...
    if (stateRef.current.running && idx > 0 && idx < proj.length) {
      const seg = LINCOLN_HEIGHTS_GPS[idx];
      const cumNow = stateRef.current.cumDist[idx];
      const normDist = cumNow / stateRef.current.maxDist;
      const normSlope = Math.min(1, Math.abs(seg.slope) / 0.2);
      const r = Math.max(2, ((1 - normDist) * 14 + normSlope * 6) / 1.5);
      let color = "#e664a0";
      if (seg.slope > 0.05) color = "#ff5028";
      else if (seg.slope < -0.05) color = "#ffbe28";
      const p = proj[idx];
    ...
    for (let i = cards.length - 1; i >= 0; i--) {
      ...
      C.age++;
      let alpha = 1;
      if (C.age < 15) alpha = C.age / 15;
      else if (C.age > C.maxAge - 60) alpha = Math.max(0, (C.maxAge - C.age) / 60);
      const p = proj[C.idx];
      const cw = 230;
      const lines = wrapText(C.news.title, 38).slice(0, 3);
      const ch = 24 + lines.length * 16 + 12;
      const cx = Math.max(10, Math.min(w - cw - 10, p.x + 30));
      const cy = Math.max(10, Math.min(h - ch - 10, p.y - ch - 20));
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.strokeStyle = "rgba(125,211,252,0.5)";
      ctx.lineWidth = 0.75;
      ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(cx + 8, cy + ch / 2); ctx.stroke();
      ctx.fillStyle = "rgba(10,5,15,0.82)";
    ...
    if (!stateRef.current.running || stateRef.current.finished) {
      drawLocationLabel(ctx);
    }
  }
  ...
  function finish() {
    ...
    if (a) {
      ...
      setTimeout(() => {
        try { a.osc.stop(); a.lfo.stop(); a.ctx.close(); } catch { /* */ }
